 ...
 * If a referenced child node does not exist, that reference is skipped. A child node can be referenced
 * multiple time and counts each time it is referenced. A metadata entry of 0 does not refer to any child node.
 *
...
const calculateSum = (item: TreeItem): number => {
	if (item.childnodes.length === 0) {
		return item.metadata.reduce((sum, metadata) => sum + metadata, 0);
	}

	return item.metadata.reduce((sum, m) => {
		const child = item.childnodes[m - 1];

		if (!child) {
			return sum;
		}

		return sum + calculateSum(child);
	}, 0);
};
...
export const challenge = (inputs: string[]): number => {
	...
	return tree.reduce((sum, child) => sum + calculateSum(child), 0);
};

 ...
 * problems - at least, that's what it looks like from the pictures in the instructions." It doesn't seem
 * like they can read whatever language it's in, but you can: "Sleigh kit. Some assembly required."
 *
 ...
 * before others can begin (your puzzle input). Each step is designated by a single letter. For example,
 ...
 * If more than one step is ready, choose the step which is first alphabetically. In this example,
...
export const challenge = (inputs: string[]): string => {
	...
	while (dependencyArray.length > 0) {
		for (const { letter, parents } of dependencyArray) {
			const dependenciesResolved = parents.every((e) => order.includes(e));
